...
Our approach is independent of the particular formalism used in the generated specification, and we currently support generating rewriting logic (Maude) and graph transformations (Groove).
\end{abstract}
...
The generation of the global behavior specification is \textit{fully automatic} and results in term rewriting rules (graph transformation rules) executable in Maude (Groove).
...
\begin{enumerate}
    \item The specification must respect the semantics of each individual behavioral model, except for interactions.
...
% Model checking
We can then use the constructed specification for consistency checking by generating a state space based on a system configuration.
...
This step is fully automated, such that it can be executed as many times as needed (indicated by the loop in \cref{fig:approach}) for different system configurations and consistency requirements (including atomic propositions) but using the same specification.
...
Then, we apply the generated specifications to check behavioral consistency for the use case and discuss the results.
...
The tables show the states, rewrites/transitions, and average runtime of a full state space exploration in the Maude and Groove-based specifications.
...
Our approach is independent of the particular formalism used in the generated specification, and we currently support generating rewriting logic (Maude) or graph transformations (Groove).
