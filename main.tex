...
(i) formulate \emph{global behavioral properties},
...
First, it contains three instances of the traffic light behavioral model, representing the three traffic lights, \textbf{A}, \textbf{B}, and \textbf{B}.
...
\subsection{System relationship model}\label{sec:system-rel-model}
...
The \textbf{second step} is to formulate behavioral consistency requirements as global behavioral properties.
...
%If the \textsf{Token} distribution is empty, the state \textsf{Terminated} is derived. Otherwise, the process is still \textsf{Running}.
The root element \textsf{ProcessSnapshot} has \textsf{subprocesses} with associated \textsf{Token}s.
...
Using the snapshot metamodels, one can only formulate local behavioral properties.
To formulate global behavioral properties, we enrich the system relationship model with the snapshot metamodels. 
...
For example, \autoref{fig:atomic_propositions} shows how to formulate the atomic propositions $A_{green}$ and $B1_{passing}$, used in the global properties in \autoref{sec:usecase}.
