...
% Come up with a system relationship model (potentially driven by the atomic properties).
% snapshot metamodel
...
Consequently, we introduce a new model type called the \emph{snapshot metamodel}, which defines the structure of a state for a given formalism (see \autoref{fig:systemRelationshipMetamodel}).
...
\subsubsection{Generate execution semantics} \label{subsec:executionSemantics}
...
Each behavioral model is typed in a behavioral metamodel, which has a snapshot metamodel (\autoref{fig:systemRelationshipMetamodel}) structurally describing its state.
...
% TODO: Describe benchmark and methodology (cite hyperfine).
\begin{table}
\centering
...
\footnotetext{The results were obtained using a machine with X processor and Y GB of RAM running Maude version 3.2.1 and Groove version 5.8.1.
A description how to run the state space exploration can be found at \cite{timkrauterBehavioralConsistencyMultimodeling2022}.}

% TODO: Add hyperfine to the repo. Benchmark should run out of the box.
% TODO: Run and fill in state data.
% TODO: Run and fill in benchmark data.
\begin{table}
\centering
\begin{tabular}{|c || c | c | c |}
 \hline
