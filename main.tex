...
% TODO: State-changing elements remark in the cover letter. See notes in the last meeting.
...
\Cref{fig:global_rule} shows the global rule resulting from the merging process.
...
A global \gls*{gt} rule is calculated by taking the sum of all left-hand sides and right-hand sides of the individual \gls*{gt} rules (implements step 2(b)) \cite[Definition 3.2.7]{baldanConcurrentSemanticsAlgebraic1999}.
...
To support a new modeling language in our approach one has to do the following tasks: describe the state structure in a snapshot metamodel, identify state-changing elements in the language's metamodel, and implement a \gls*{hot} to the chosen formalism.
...
These tasks are typically done by a language engineer which may be a different person than the modeler using our approach.
Generally, we assume to roles: language engineer and modeler when adopting our approach.
The modeler is in control of the use case and executes the manual tasks of the behavioral consistency management workflow (see \autoref{fig:approach}), while the language engineer is responsible for supporting all new modeling languages as described here.
