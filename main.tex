...
To address this challenge, model-driven software engineering utilizes abstract representations of the constituting systems and their interactions, resulting in a \emph{multi-model} representing the overall system.
...
(iii) align the individual models by specifying \emph{how they interact},
...
Such a collection of inter-related models is referred to as a \emph{multi-model} \cite{boronatWhatMultimodelingLanguage2009}, which is usually heterogeneous, meaning it consists of models conforming to different modeling languages.
...
Thus, current approaches define so-called \emph{commonalities} to explicate these relationships and keep the information consistent.
...
Structural consistency can then be verified using the global view.
...
Our approach applies to behavioral formalisms where state variables are discrete (see related work in \cref{sec:related_work}).
...
The generation of a global behavior specification is similar to merging structural models into a global view.
To achieve a global view for structural models they introduce a base language in which individual models and the global view can be represented.
Likewise, generating our global behavior specification is based on the two key concepts of state and state-changing elements.
Generally, we adapt the three steps for structural consistency management: Alignment, Verification and Reconciliation as proposed in \cite{stunkelComprehensiveSystemsFormal2021} to a multi-model containing behavioral models.
...
Afterward, we show how we can use the graph transformation toolset Groove to check behavioral consistency (\autoref{sec:specification_of_the_global_behavior}).
Furthermore, we discuss the inevitable state space explosion problem in \autoref{sec:state_space_explosion}.
...
\begin{enumerate}
    \item We define a \emph{\glsfirst*{srm}} describing which behavioral models may interact.
...
Thus, lifting property specification to the domain-specific level is a promising idea that fits into our approach.
...
We call behavioral inter-model relationships \emph{interactions} since they carry behavioral meaning while commonalities carry structural meaning \cite{stunkelComprehensiveSystemsFormal2021, klareCommonalitiesPreservingConsistency2019}.
...
Interactions are specific to each use case but identifying state-changing elements for a behavioral language must only be done once.
...
In \cref{sec:specification_of_the_global_behavior}, we describe how we generate specifications for the graph transformation toolset Groove.
...
In this section, we will describe our implementation to generate formal \gls*{gt} specifications in Groove \cite{rensinkGROOVESimulatorTool2004}.
Then, we use the generated specification to check behavioral consistency for the use case and discuss the results.
% focus on groove . visual rules are more understandable
We have chosen to use the \gls*{gt} formalism to describe our approach because \gls*{gt} provide a visual representation of the rules and allow for a clear understanding.
% Maude description can be found online.
...
Both implementations utilize a \emph{global} \gls*{hot} from the behavioral models and their interactions to \gls*{gt} rules (Groove) or term rewriting rules (Maude).
...
We will now generally describe this \gls*{hot} using \emph{rule} to mean either \gls*{gt} or term rewriting rule depending on the target formalism.

The \gls*{hot} can be decomposed into two steps.
The first step to generate a specification of the global behavior is to create rules for each behavioral model contained in the multi-model.
...
Thus, we need \emph{local} \gls*{hot}s for each behavioral modeling language producing rules.
...
\Cref{fig:global_rule} shows the global rule resulting from the merging process for graph transformation rules.
...
In the following sections, we describe the local \glspl*{hot} to \gls*{gt} rules and how global rules are created from the multiple \gls*{gt} rules.
...
% This is possible since a class diagram can be seen as an attributed type graph with inheritance \cite{heckelGraphTransformationSoftware2020}.
...
% cite a potential paper explaining the BPMN semantics here.
...
Running the obtained \gls*{gt} specification shows that properties \eqref{eq:property1} and \eqref{eq:property2} hold, while properties \eqref{eq:property3} and \eqref{eq:property4} do \emph{not} hold (see artifacts in \cite{krauterArtifactsBehavioralConsistency2023}).
...
However, we refined the behavioral consistency management workflow into five steps and introduced new key concepts such as interactions, behavioral relationships, and state-changing elements (see \gls*{srm}).

% Intro to the following sections
First, we discuss ad-hoc solutions to the problem of behavioral consistency.
Then, we look at a general framework to compose heterogeneous models.
Afterwards, we relate our work to the fields of \gls*{mpm} and co-simulation.
Finally, we refer to the literature on behavioral interfaces and coordination languages.
...
Nevertheless, the approaches only resemble ad-hoc solutions to specific combinations of two languages.
...
Thus, one could also experiment with Event structures besides the two currently implemented formalisms.
Since, the research challenges they identify and the resulting work items align well with our work, we see our work as a continuation of \cite{kienzleUnifyingFrameworkHomogeneous2019}.
In addition, they cover the same class of behavioral formalisms as our approach (see \acrshort*{devs} in the next subsection).
However, they focus on composition which is only the first step towards behavioral consistency checking.
% They also only cover DEVS

\subsection{Multi-paradigm modeling and co-simulation}
% Introduce MPM. 
\gls*{mpm} is based on the idea to model every aspect and part of a system using the most appropriate modelling formalism(s) \cite{amraniMultiparadigmModellingCyber2021}.
...
\Cref{fig:classification} shows the classification where time and state variables can either be \textit{discrete} or \textit{continous}.
...
We think that supporting \gls*{devs} formalisms is a good trade off between usefulness and complexity of our approach, since \gls*{devs} covers a wide variety of formalisms \cite{vangheluweIntroductionMultiparadigmModelling2002}.

% transition into co-simulation by motivating it
Due to the use of different modelling formalisms system simulation when following the \gls*{mpm} paradigm is not an easy task.
...
% Ptolemy is one co-simulation approach
...
% Their goal is simulation not behavioral consistency checking.
However, with co-simulations, one can only simulate systems not check global behavioral properties.
Our approach supports this by introducing a minimal behavioral interface (state and state-changing elements) and only allowing formalisms with discrete state variables.
\subsection{Behavioral interfaces and coordination}
...
Previous work either only dealt with the behavioral consistency between specific pairs of models or focused on the simulation in a heterogeneous scenario.
% Reiterate the novelty regarding a minimal behavioral interface for consistency checking and interactions!
