...
Using the \gls*{srm} and snapshot metamodels, we can represent the global states of the system, but we still need execution semantics to check the global behavioral properties.
The execution semantics used in our approach must fulfill the following three requirements:
\begin{enumerate}
    \item The execution semantics must respect the semantics of each individual behavioral model, except for interactions.
    \item The execution semantics must reflect the defined interactions between the behavioral models.
    \item The execution semantics must allow the checking of behavioral properties for a given system configuration.
\end{enumerate}
Thus, any execution engine fulfilling these three requirements can be used in our approach.
Consequently, one can experiment with different underlying execution engines, for example, graph transformations, rewriting logic, state machines, Petri nets, or process algebras, without changing the general framework.
...
In \cref{sec:global_execution_semantics}, we describe implementations for two possible execution engines, namely the graph transformation toolset Groove and the Maude system.
...
However, execution semantics can be reused for all defined properties using various atomic propositions.
...
This step is fully automated, such that it can be executed as many times as needed (indicated by the loop in \cref{fig:approach}) for different system configurations and consistency requirements (including atomic propositions) but using the same execution semantics.
...
In addition, we add behavioral relationships from the SRM such that we can instantiate them to create the system configuration described in the use case.
