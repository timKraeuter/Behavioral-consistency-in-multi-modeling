...
However, there is no approach to check \textit{consistency} of \textit{arbitrary many} behavioral models.
...
These heterogeneous behavioral formalisms can be classified using two criteria: \textit{time} and \textit{values of state variables} \cite{wainerDiscreteeventModelingSimulation2009}.
...
\begin{figure}[ht!]
    ...
    \caption{Classification of behavioral formalisms according to the nature of the \textit{time} and \textit{state variables} \cite{wainerDiscreteeventModelingSimulation2009, amraniMultiparadigmModellingCyber2021}}
    \label{fig:classification}
\end{figure}

% We only support formalisms where states are discrete (lower part).
So far we have seen \glspl*{dtds} like \gls*{fsm} and \gls*{bpmn} in this work, where time and states are discrete.
...
Co-Simulation \cite{gomesCoSimulationSurvey2019}
...
% FMI/FMU standard should be named. check leroy paper

% Their goal is simulation not model-checking. Which is probably even harder in continous models.
...
\subsection{Relations to structural consistency}
% Needed or expand in the introduction?
