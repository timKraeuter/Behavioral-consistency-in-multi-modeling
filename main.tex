...
Both implementations utilize a \emph{global model transformation} from the behavioral models and their interactions to \gls*{gt} rules (Groove) or rewriting rules (Maude).
We will now describe this transformation generally using the general term \emph{rules} to mean either \gls*{gt} rules or rewriting rules.
...
Concretely, we utilize the Groove tool set to run the generated specifications, i.e., \gls*{gt} systems \cite{ghamarianModellingAnalysisUsing2012, rensinkGROOVESimulatorTool2004}.
...
To apply our approach to the use case, we need to define local model transformations from state machines and \gls*{bpmn} processes to \gls*{gt} rules.


\subsubsection{State machine semantics}
The local model transformation to generate \gls*{gt} rules for finite state machines is straightforward.
...
The local model transformation to generate \gls*{gt} rules for \gls*{bpmn} processes is challenging, and we are currently only supporting a subset of the \gls*{bpmn} semantics \cite{krauterFormalizationAnalysisBPMN2022}, similar to the extent supported by \cite{vangorpVisualTokenbasedFormalization2013}.
