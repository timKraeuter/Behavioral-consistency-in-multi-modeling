...
\jotdetails{
    volume=22,      % volume
    number=2,      % number or issue
    articleno=a9,   % article number, eg a1 for research articles, e for editorials
    year=2023,      % year
...
\author[$\ast$]{Tim Kr\"{a}uter}
% Arbitrary co-author order. Everyone contributed significantly.
\author[$\dagger\ast$]{Harald K\"{o}nig}
\author[$\ast$]{Adrian Rutle}
\author[$\ast$]{Yngve Lamo}
\author[$\ddagger$]{Patrick St\"{u}nkel}

\affil[$\ast$]{Western Norway University of Applied Sciences, Norway}
\affil[$\dagger$]{University of Applied Sciences FHDW Hannover, Germany}
\affil[$\ddagger$]{Haukeland Universitetssykehus, Norway}
...
\runningtitle{Behavioral consistency in multi-modeling} % For use in the internal pages 
...
Our approach is decoupled from the particular formalism used in the generated formal specification, and we currently support graph transformations (Groove) and rewriting logic (Maude).
\end{abstract}
...
Moreover, an early green for traffic light B can be requested.
...
The full model and all other models are available in \cite{krauterArtifactsECMFA20232023}.
...
As an alternative to \gls*{gt} and Groove, we describe in \cite{krauterArtifactsECMFA20232023} our implementation using rewriting logic and Maude.
...
We generate typed \gls*{gt} systems, where the merge of the \gls*{srm} and the snapshot metamodels is the type graph \cite{krauterArtifactsECMFA20232023}.
...
The generated \gls*{gt} system and the other \gls*{gt} systems of the use case, including further instructions regarding execution and consistency checking, can be found in \cite{krauterArtifactsECMFA20232023}.


\subsubsection{BPMN semantics}
The local \gls*{hot} to generate \gls*{gt} rules for \gls*{bpmn} processes is challenging, and we are currently only supporting a subset of the \gls*{bpmn} semantics (see \cite{krauterArtifactsECMFA20232023}).
Generally, we construct one or more rules for each flow node, i.e., type of state-changing element in a \gls*{bpmn} model.
Furthermore, we created a comprehensive test suite to ensure the correctness of our \gls*{hot} \cite{krauterArtifactsECMFA20232023}.
...
Due to limited space, we only show the \textsf{Switch to P1} rules but the artifacts of this paper \cite{krauterArtifactsECMFA20232023} contain the full \gls*{gt} system and a wiki explaining the \gls*{hot} in detail.
...
Finally, we can generate the global state space of the system using the global \gls*{gt} system, which can be found in \cite{krauterArtifactsECMFA20232023}.
...
Running the obtained \gls*{gt} specification shows that properties \eqref{eq:property1} and \eqref{eq:property2} hold, while properties \eqref{eq:property3} and \eqref{eq:property4} do \textit{not} hold (see artifacts in \cite{krauterArtifactsECMFA20232023}).
...
A description of how to run the benchmarks is available in \cite{krauterArtifactsECMFA20232023}.
...
% Could be deleted but I think it's quite nice.
