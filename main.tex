...
The generation of the global behavior specification is \emph{fully automatic} and results in \gls*{gt} rules (or respectively, term rewriting rules) executable in Groove (Maude).
...
The interaction of behavioral models is only possible through instances of the state-changing elements.
Thus, state-changing elements function as a minimal \textit{behavioral interface} to uniformly define interactions for heterogeneous behavioral languages.
...
Generally, models behave in a distributed independent fashion until they reach a state-changing element which is part of an interaction.
...
The successful implementation serves as a \emph{proof of concept}.
...
Furthermore, we created a large test suite to ensure correctness of our \gls*{hot} \cite{krauterArtifactsBehavioralConsistency2023}.
...
First, we discuss ad-hoc and more general solutions to the problem of behavioral consistency.
...
Since, their research challenges and resulting work items align well with our work, we see our work following the same line of research.
In addition, they cover the same class of behavioral formalisms as our approach (see \acrshort*{devs} in the next subsection).

\cite{varalarsenBCOolBehavioralCoordination2016} propose the coordination language B-COoL.
In B-COoL a user defines a behavioral interface for each behavioral language, which are then used to specify interactions between models.
...
In this paper, we identified the concept of \textit{state} and the minimal behavioral interface of \textit{state-changing elements} as the two necessary factors to check behavioral consistency when heterogeneous models interact.
...
% Conclusion
Our work represents the first formalization of behavioral consistency management in a heterogeneous modeling scenario, enabling formulating and checking \emph{global} properties.
...
Furthermore, state-changing elements serve as a minimal behavioral interface to uniformly define interactions between heterogeneous models.
