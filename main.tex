...
We have chosen to use the \gls*{gt} formalism to describe our approach.
\gls*{gt} provide a visual representation of the rules and allow for a clear understanding.
...
The generated Maude specifications and example executions can be found in \cite{krauterArtifactsBehavioralConsistency2023}.
...
\Cref{lst:tJunctionControllerMaude} shows the Maude rewriting rules for the task \textsf{Switch to P1} of the TJunction controller.
...
float=*t,
belowskip=-1.5 \baselineskip,
caption=Example rewriting rules for the TJunction controller]{figures/tJunctionController.maude}

Like the \gls*{gt} rules, the rewriting rules move tokens by changing their position according to the given \gls*{bpmn} model.
Thus, we can generate Maude rewriting rules that implement the behavioral semantics of the \gls*{bpmn} models in the use case (see \cite{krauterArtifactsBehavioralConsistency2023}).


\subsubsection{Check behavioral consistency}
The defined interactions change the rules for switching to phases 1 and 2.
\Cref{lst:synchRuleMaude} shows the resulting rule for switching to phase 1, where synchronization with the end of the task was chosen.
...
Both specifications show that properties \eqref{eq:property1} and \eqref{eq:property2} hold, while properties \eqref{eq:property3} and \eqref{eq:property4} do \emph{not} hold (see artifacts in \cite{krauterArtifactsBehavioralConsistency2023}).
