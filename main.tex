...
\autoref{fig:t_junction} shows how a possible controller for a T-Junction behaves in the traffic management system\footnote{All models and their source files can be found in \cite{timkrauterArtifactsBehavioralConsistency2022}.\label{footnote:fullModels}}.
...
\begin{figure}[h]
    ...
    \caption{Graph transformation rules for the T-Junction controller}
...
\begin{figure}[h]
    ...
    \caption{Parallel graph transformation rule to switch to phase 1}
...
% TODO: Describe rule merges. Merging both sides and adding behavioral links.
TODO: Describe the rule merge in Maude.
...
Using Maude's search command and search graph visualization, we can verify that the generated state space conforms to the traffic light state machine\footnote{The corresponding Maude files and example Maude executions can be found in \cite{timkrauterArtifactsBehavioralConsistency2022}}.
...
\autoref{lst:tJunctionControllerMaude} shows rewriting rules for the same flow nodes of the T-Junction controller as depicted in \autoref{fig:bpmn_example_rules}.

\lstinputlisting[
label=lst:tJunctionControllerMaude,
language=Interaction,
float=*,
caption=Rewriting rules for the T-Junction controller]{figures/tJunctionController.maude}
...
Both implementations show that properties 1 and 2 hold, while properties 3 and 4 do not hold\cref{footnote:GGinRepo}.
...
\begin{tabular}{|c || c | c | c |}
 ...
 \hline\hline
 No buses & 168 & 663 & \textasciitilde 67.6 ms \\
 \hline
 1 bus & 3.304 & 20.117 & \textasciitilde 277.5 ms \\
 \hline
 2 buses & 35.280 & 28.2975 & \textasciitilde 3.008 ms \\
 \hline
 3 buses & 260.176 & 2.555.701 & \textasciitilde 27.453 ms \\
 \hline
\end{tabular}
...
\footnotetext{\label{footnote:benchmark}A description how to run the benchmarks can be found in \cite{timkrauterArtifactsBehavioralConsistency2022}.}
