...
% Create a \gls*{srm} (driven by the atomic properties and potential interactions).
...
\begin{enumerate}
    ...
    \item The execution semantics must generate a state space, where given a system configuration---i.e. an instance of the \gls*{srm}---each state can be interpreted as an instance of the \gls*{srm} and the corresponding snapshot metamodels. % Allow model checking of properties defined using the snapshot metamodels and SRM
\end{enumerate}
...
% Should also include Groove. Check for repetition.
