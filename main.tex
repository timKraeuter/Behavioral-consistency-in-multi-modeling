...
% TODO: Shortly introduce graph transformation systems (also attributed/typed)
The idea of a graph transformation system is to begin with a start graph and then apply all possible graph transformation rules.
...
As a type graph for the graph transformation system, we utilize the \gls*{srm} including the snapshot metamodels which fits rules coming from individual models and rules created by interactions.
...
Together with a system configuration, the set of typed global rules results in a graph transformation system generating a state space.
...
Using a traffic light snapshot with the state red as a start graph results in a graph transformation system\footnote{\label{footnote:GGinRepo}All generated graph transformation systems, including further instructions regarding execution and consistency checking, can be found in \cite{krauterArtifactsBehavioralConsistency2022}.}, which will generate the same state space as the traffic light state machine.
We evaluated our approach by executing the graph transformation system with the graph transformation tool Groove \cite{ghamarianModellingAnalysisUsing2012, rensinkGROOVESimulatorTool2004}.
...
Finally, we can generate the global state space of the system using the global graph transformation system\cref{footnote:GGinRepo}.
