...
Furthermore, line 3 specifies that one of the synchronization partners is the element \emph{turn green} connected by the relationship \emph{A}.
...
Both implementations utilize a \emph{global higher-order model transformation} from the behavioral models and their interactions to \gls*{gt} rules (Groove) or rewriting rules (Maude).
...
The higher-order model transformation can be decomposed into two steps.
...
Thus, we need \emph{local higher-order model transformations} for each behavioral modeling language producing sets of rules.
Each local higher-order model transformation only has to be implemented once by an \gls*{mde} tool developer and made available, for example, as a plugin, such that it can then be reused in any future setting the language is needed.
In addition, each local higher-order model transformation must keep traces of the generated set of rules.
...
To apply our approach to the use case, we need to define local higher-order model transformations from state machines and \gls*{bpmn} processes to \gls*{gt} rules.


\subsubsection{State machine semantics}
The local higher-order model transformation to generate \gls*{gt} rules for finite state machines is straightforward.
...
\begin{figure}[h]
    ...
    \caption{Example \gls*{gt} rules for the T-Junction controller}
...
To apply our approach to the use case, we need to define local higher-order model transformations from state machines and \gls*{bpmn} processes to rewriting rules.
...
\autoref{lst:tJunctionControllerMaude} in the appendix shows two Maude rewriting rules for the first two flow nodes of the T-Junction controller as depicted in \cref{fig:bpmn_example_rules}.
