...
Also for the snapshot metamodel of \gls*{bpmn} we reuse the concrete syntax of the corresponding \gls*{bpmn} metamodel. 
...
%by resolving the proxy links in the using must merge all snapshot metamodels and the system relationship model into one model, describing a state/snapshot of the multi-model.
...
% Each behavioral model is typed by a behavioral metamodel, which has a snapshot metamodel structurally describing its state.
...
% Merge example
% \autoref{fig:global_snapshot_model} depicts the global snapshot metamodel $M_s^+$ for the use case, describing the states of the traffic management system.
