...
The full model and all other models are available from \cite{krauterArtifactsBehavioralConsistency2023}.
...
\end{enumerate}
The behavioral consistency management workflow potentially uncovers violations, describing counterexamples in which the global behavioral properties are not valid.
...
% Maude description can be found here online.
...
Due to limited space, we only show the \textsf{Switch to P1} rules but the artifacts of this paper \cite{krauterArtifactsBehavioralConsistency2023} contain the full \gls*{gt} system.
...
% Add some intro to the following sections

% First ad-hoc solutions --> Has checking but ad-hoc
% Model composition in MDE --> Has no checking
% MPM + Co-Simulation: Ptolemy and TBD overview paper from Harald. --> Has no checking
% Behavioral interfaces and coordination. Interactions achieved differently
...
Consequently, they only solve specific problems with ad-hoc solutions but do not consider the general problem of behavioral consistency in a heterogeneous modeling scenario.
