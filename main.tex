...
Finally, we discuss the inevitable state space explosion problem in \autoref{sec:state_space_explosion} and conclude in \autoref{sec:conclusion_and_future_work}.
...
% First ad-hoc
% General solutions? Problem: Heterogeneous model composiotion
% MPM: The MPM part nicely scopes my work.
% Co-Simulatin: Ptolemy and TBD overview paper from Harald.
% Behavioral interfaces and coordination?
...
Furthermore, our approach can also support \glspl*{devs} since the underlying formalisms support continuous time, see for example \cite{olveczkyRealTimeMaudeIts2014}.

However, we do not support formalisms with continuous state variables (top part of \cref{fig:classification}), since the central concept of \textit{state-changing elements} would need to be changed.
...
% I think Ptolemy fits Co-Similuation. In addition fmi/fmu. check leroy paper
