...
Furthermore, we discuss the inevitable state space explosion problem in \autoref{subsec:state_space_explosion}.
...
We will call the resulting models \emph{snapshot metamodels} and specify them using class diagrams.
...
Since instances of snapshot metamodels are essentially graphs they can be matched to a given system state in a uniform way.
...
In addition, we introduce the second fundamental concept of \emph{state-changing elements}.
...
\section{Discussion}
In this section we discuss the following aspects regarding our approach: semantic gaps, how to support new modeling languages, and the state space explosion problem.
...
We learned that when implementing \glspl*{hot} to the same underlying formalism, one gets accustomed to the formalism and automatically builds a framework to generate and test specifications in this formalism.
...
\subsection{State space explosion} \label{subsec:state_space_explosion}
