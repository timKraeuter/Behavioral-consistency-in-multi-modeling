...
(iii) check the \emph{global behavioral properties} which should be satisfied by the multi-model by generating an implementation of the specified \emph{global behavior}.
Our approach is independent of the particular formalism used in the generated implementation, and we currently support generating rewriting logic (Maude) and graph transformations (Groove).
\end{abstract}
...
Our approach facilitates specifying \emph{interactions} between multiple potentially heterogeneous behavioral models, which are in turn used to generate an implementation of the global behavior.
Our approach is independent of the particular formalism used in the generated implementation, and we currently two formalisms.
The generation of the global behavior implementation is \textit{fully automatic} and results in term rewriting rules (graph transformation rules) executable in Maude (Groove).
...
Afterward, we show how we can use the Groove tool set and the Maude system to check behavioral consistency (\autoref{sec:implementation_of_the_global_behavior}).
...
\begin{enumerate}
    ...
    \item We automatically generate an implementation of the specified global behavior using the interactions and the \gls*{srm}.
...
% TODO finalize title and align with the workflow

Using the \gls*{srm} and snapshot metamodels, we can represent the global states of the system, but we still need an executable implementation of the global behavior to check the defined properties.
The implementation of the global behavior used in our approach must fulfill the following three requirements:
\begin{enumerate}
    \item The implementation must respect the semantics of each individual behavioral model, except for interactions.
    \item The implementation semantics must reflect the defined interactions between the behavioral models.
    \item The implementation semantics must allow the checking of behavioral properties for a given system configuration.
\end{enumerate}
Thus, any implementation in a given formalism fulfilling these three requirements can be used in our approach.
...
In \cref{sec:implementation_of_the_global_behavior}, we describe how we generate implementations for the graph transformation toolset Groove and the Maude system.

To summarize, we generate an implementation of the global system behavior.
This generation takes the the models and interactions as input and is fully automated to allow frequent model changes.
However, generated implementations can be reused for all defined properties.


\subsection{Check behavioral consistency}
% TODO: M_s+ does not exist anymore!
% Model checking
We can then use the constructed implementation for consistency checking by generating a state space based on a system configuration.
...
This step is fully automated, such that it can be executed as many times as needed (indicated by the loop in \cref{fig:approach}) for different system configurations and consistency requirements (including atomic propositions) but using the same implementations.
...
An unsuccessful consistency check leads to a consistency restoration process, which is crucial but out of scope for this paper.
We describe consistency checking for the use case and its result in the end of the next section.


\section{Specification of the global behavior} \label{sec:implementation_of_the_global_behavior}
...
The first step to generate an implementation of the global behavior is to generate rules for each behavioral model contained in the multi-model.
...
Finally, we can check the global consistency of the system in different configurations with the given consistency requirements by generating an implementation of the global behavior.
The global behavior implementation is based on the defined interactions but preserve the original behavior of each model besides interactions.
...
Although our approach is independent of the particular formalism used in the generated implementation, the current two implementations utilize rewriting logic (Maude) or graph transformations (Groove).
